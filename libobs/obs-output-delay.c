...
bool obs_output_delay_start(obs_output_t *output)
{
	...
	if (output->delay_active) {
		...
		obs_output_signal_delay(output, "delay_starting");
		return true;
	}
...
void obs_output_delay_stop(obs_output_t *output)
{
	...
	obs_output_signal_delay(output, "delay_stopping");
}
		...
		uint32_t flags)
{
	...
	if ((output->info.flags & OBS_OUTPUT_DELAYABLE) == 0) {
		blog(LOG_WARNING, "Tried to set a delay value on a "
		                  "non-delayable output");
		return;
	}
